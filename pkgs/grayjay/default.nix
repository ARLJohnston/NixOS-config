...
(pkgs.buildFHSEnv rec {
  ...
  installDir = "/home/config/pkgs/grayjay";

  # grayjay script wrapper that sets cwd to `installdir'
  start-grayjay =
    pkgs.writeShellScriptBin "start-grayjay" "cd ${installDir} && ./Grayjay";
  ...
  grayjay-desktop-file = pkgs.makeDesktopItem {
    ...
    comment =
      "A desktop client for Grayjay to stream and download video content";
